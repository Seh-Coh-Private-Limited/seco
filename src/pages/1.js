...
Format each suggestion on a new line.`;
  ...
  const fetchSuggestions = async () => {
    ...
    try {
      const prompt = generatePrompt(questionType, currentQuestion?.question, historicalResponses);
      if (!prompt) {
...
const AutoResizeTextarea = ({ value, onChange, onKeyPress, placeholder, disabled }) => {
  ...
  // Adjust height only when necessary and preserve focus
  useEffect(() => {
...
const ChatInput = ({
  ...
  isLoading,
  hasSeenTypewriter,
  handleVoiceRecord, // Assuming this is passed from parent
  handleFileUpload,  // Assuming this is passed from parent
  getCurrentQuestionType, // Assuming this is passed from parent
  recognition,       // Assuming this is passed from parent
  hasSubmitted,
}) => {
  const textareaRef = useRef(null);
        ...
        <div className="flex justify-between items-center">
          <div className="flex gap-2">
            <button
              ...
              className={`p-3 bg-white border rounded-full transition-colors duration-200
                ...
                ${!hasSeenTypewriter || !recognition || hasSubmitted ? 'opacity-50 cursor-not-allowed' : ''}`}
            ...
            <label
              className={`p-3 bg-white border rounded-full transition-colors duration-200
                ${getCurrentQuestionType() === 'fileUpload' && !isLoading && hasSeenTypewriter && !hasSubmitted
          ...
          <button
            ...
            className={`p-3 bg-white border rounded-lg transition-colors duration-200
              ${!isLoading && inputValue.trim() && hasSeenTypewriter && !hasSubmitted ? 'hover:bg-gray-50' : 'opacity-50 cursor-not-allowed'}`}
          >
            <Send className="h-6 w-6" />
          </button>
        </div>
