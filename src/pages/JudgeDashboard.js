...
import JudgesFormResponses from './JudgesFormResponses';
  ...
  const JudgeDashboard = () => {
    ...
    const fetchCompanyDetails = async (user) => {
        try {
          // Query Firestore to find the document with the email field
          const q = query(
    ...
    const fetchProgrammes = async (user) => {
        try {
          // Step 1: Get the program ID from the 'judges' collection using the session email
          const judgesQuery = await getDocs(
            query(
              collection(db, 'judges'),
              where('email', '==', user.email) // Assuming sessionEmail is stored in user object
            )
          );
          
          if (judgesQuery.empty) {
            setError('No judge found for this session email');
            setProgrammes([]);
            return;
          }
      
          // Assuming there's only one judge document for the sessionEmail
          const judgeData = judgesQuery.docs[0].data();
          const programId = judgeData.programId; // Replace 'programId' with the actual field name in 'judges'
      
          // Step 2: Get the programmes from 'programmes' collection based on the programId
          const programmesQuery = await getDocs(
            query(
              collection(db, 'programmes'),
              
              
              where('id', '==', programId) // Match with the fetched programId
            )
          );
